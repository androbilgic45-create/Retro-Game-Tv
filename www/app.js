...
// Android/media/<paket> yolu, Android/data'nın aksine Android 11+ scoped
// storage kısıtlarında diğer dosya yöneticileri tarafından hâlâ görülebilir
// kalıyor — kullanıcı USB/dosya yöneticisiyle ROM atabilsin diye bu tercih edildi.
const ROOT_HINT_PATH = `Android/media/${APP_ID}/roms`;
...
const RomScanner = {
  ...
  async _scanNative(system){
    ...
    try{
      ...
      // Klasör henüz yok — sorun değil, boş liste döneriz.
      return [];
    }
  },
  ...
  async _ensureFolders(){
    ...
    for(const sys of SYSTEMS){
      ...
      }catch(e){ /* zaten varsa hata verir, göz ardı */ }
    }
  },
...
const GamepadManager = {
  ...
  _poll(){
    ...
    for(const gp of pads){
      ...
      // Kenar tetiklemeli butonlar (basılı tutmada tekrar etmesin)
      const pressedNow = (i) => cur[i] && !prev[i];
      if(info.slot === 1){
        if(pressedNow(0)) this.onAction("confirm", info); // A / Cross
        if(pressedNow(1)) this.onAction("back", info);    // B / Circle
        if(pressedNow(3)) this.onAction("menu", info);    // Y / Triangle -> alt menü
        if(pressedNow(2)) this.onAction("quickFav", info);// X / Square -> hızlı favori
...
function renderSettingsList(entry){
  ...
  const items = [
    ...
    { label:"Kumanda eşleme", sub: GamepadManager.hasAny() ? "Kumanda bağlı" : "Kumanda bağlı değil", action:"none" },
  ];
  items.forEach((it, i) => {
    const row = document.createElement("div");
    row.className = "list-item" + (i === App.listIndex && App.zone === "list" ? " focused" : "");
...
function handleSettingsConfirm(item){
  ...
  // 'info' ve 'none' aksiyonları şimdilik bilgi amaçlı, ek işlem gerekmiyor.
}
